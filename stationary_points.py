...
def half_circle_init(dim):
    """Returns theta that generates positive X for sure."""

    return np.random.uniform(-np.pi / 4, np.pi / 4, size=dim)


def half_circle_mat_init(dim):
...
if __name__ == '__main__':
    ...
    # generated by restraining the range of theta)
    while True:
        A, z = basin.get_observation(10, 1, 'positive-sparse')
        dim, _ = A.shape
        curv = largest_nonzero_curv(trig.trig_hess(A, half_circle_init(dim)))
        print("The curvature is: {}".format(np.real(curv)))
        if curv > 0:
            exit(0)
